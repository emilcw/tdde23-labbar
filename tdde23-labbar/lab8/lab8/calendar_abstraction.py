...
def show_free_spans(cal_day, start, end):
    search_span = new_time_span(start,end)
    a = new_time_spans()
    print ("cal day", cal_day)
    for elem in strip_tag(cal_day):
        pass
...
def overlap(ts1,ts2):
    ...
    if are_overlapping(ts1,ts2):
        ...
        print ("hour1",hour1)
        print ("hour2", minute1)
        return ('span', (('time', (('hour', hour1), ('minute', minute1))), ('time', (('hour', hour2), ('minute', minute2)))))
...
def length_of_span(ts):
    "span -> duration"
    duration = (strip_tag(strip_tag(end_time(ts))[0]) - strip_tag(strip_tag(start_time(ts))[0]), strip_tag(strip_tag(end_time(ts))[1]) - strip_tag(strip_tag(start_time(ts))[1]))
